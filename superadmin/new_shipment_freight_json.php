...
//==================================================== WITHOUT HAZMAT (Regulated)
        ...
        curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($data_freight));
            //=====print a json file
            $path = 'file.json';
            $jsonString = json_encode($data_freight, JSON_PRETTY_PRINT);
            // Write in the file
            $fp = fopen($path, 'w');
...
if($hazmat=='yes' && $h_hazmatType=='Regulated' && $appointment_type=='None'){
    curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($data_regulated_hazmat_scnone));
        //=====print a json file
        $path = 'file.json';
        $jsonString = json_encode($data_regulated_hazmat_scnone, JSON_PRETTY_PRINT);
        // Write in the file
        $fp = fopen($path, 'w');
        fwrite($fp, $jsonString);
        fclose($fp);
